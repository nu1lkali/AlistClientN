...
- 🎲 **Emby 随机播放**（v1.4.4 新增）：支持多 Emby 服务器/媒体库管理，从媒体库一键随机抽取视频进入“视界流”上下滑动播放，支持对媒体进行标记
...
### 收藏夹
- **多收藏夹管理**：创建、重命名、删除自定义收藏夹，文件分类整理
- **侧边面板导航**：左侧收藏夹列表，快速切换不同收藏夹，宽屏主从布局 / 窄屏抽屉模式自适应
- **默认收藏夹**：可设置默认收藏夹，新收藏默认存入
- **文件移动**：支持将收藏文件在不同收藏夹之间移动
- **全部收藏视图**：查看所有收藏夹中的全部文件
- **批量清空**：一键清空指定收藏夹的全部内容
- 随机打开收藏的图片
- 随机播放收藏的视频
- 左滑删除 / 更多菜单操作
...
| <img src="https://img.erpweb.eu.org/imgs/2026/04/64442e665e6bca3b.jpg" width="250"> | <img src="https://img.erpweb.eu.org/imgs/2026/09/b861f343f7b6fa01.jpg" width="250"> | <img src="https://img.erpweb.eu.org/imgs/2026/09/c3e2aba87f3149ef.jpg" width="250"> |
| <img src="https://img.erpweb.eu.org/imgs/2026/09/7f705668002be713.jpg" width="250"> | <img src="https://img.erpweb.eu.org/imgs/2026/09/b04151a84b08e5a6.jpg" width="250"> | <img src="https://img.erpweb.eu.org/imgs/2026/09/65ec47fe6313205f.jpg" width="250"> |
...
### v1.4.4
- **Emby 随机播放**（新功能）：从 Emby 服务器媒体库随机抽取视频，进入“视界流”上下滑动播放
  - 设置页新增「Emby 随机播放」分组：Emby 服务器管理 / 媒体库管理 / 每次随机数量 / 使用说明
  - 多服务器管理：备注名、协议(http/https)、地址端口、API Key 掩码，支持一键“测试连接”(GET /Users)，点选切换主服务器
  - 媒体库一键拉取：GET /Users/{userId}/Views 拉出全部媒体库（名称/Id/类型），点选即添加，无需手工查询 ParentId
  - 手动填写 ParentId 时可按 Id 反查媒体库名称并自动填入备注
  - 首页文件列表右上角新增「随机播放」按钮：自动获取用户并缓存 → SortBy=Random 随机抽取 → 拼接播放直链 → 无缝进入“视界流”
  - 服务器/媒体库配置本地持久化(SharedPreferences)，切换后即时生效，无需重启 App
  - 支持对媒体库媒体进行收藏/标记，调用Emby接口。

### v1.4.3
- **多收藏夹管理**：收藏页新增侧边面板，支持创建、重命名、删除自定义收藏夹
  - 数据库新增 `favorite_folder` 表，`favorite` 表新增 `folder_id` 字段关联收藏夹
  - 默认收藏夹：首次启动自动创建"默认收藏夹"，已有收藏自动归入
  - 可设置任意收藏夹为默认，新收藏默认存入
  - 收藏文件可在不同收藏夹之间移动
  - 宽屏主从布局 / 窄屏抽屉模式自适应
  - 全部收藏视图：查看所有收藏夹中的全部文件
  - 批量清空：一键清空指定收藏夹的全部内容
  - 长按收藏夹弹出菜单：重命名、设为默认、删除
- **收藏夹面板 UI 优化**：顶部圆角卡片设计、状态栏自适应间距、标题升级为 titleLarge、图标优化
